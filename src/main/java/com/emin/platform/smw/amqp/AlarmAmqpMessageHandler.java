 ...
 * @since 1.0.0
 */
public class AlarmAmqpMessageHandler implements AmqpMessageHandler {

    /**
     * 日志
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(AlarmAmqpMessageHandler.class);
    ...
    public boolean isMatcherHandler(AmqpCommonStructure<AmqpNotifyMessageStructure<JSONObject>> notify) {
        ...
        return ApplicationConstain.AmqpDtoCategoryEventCode.NOTIFY_MESSAGE.getCode().equals(notify.getDtoCategoryId())
                &&
                ApplicationConstain.AmqpNotifyMessageEventCode.ALARM.getCode().equals(notify.getData().getEventCode());
    }
    ...
    public void handler(AmqpCommonStructure<AmqpNotifyMessageStructure<JSONObject>> notify) {
        ...
        try {
            NotifyWebSocketServer.broadcastNotifyType(eventSourceId + "",
                    ApplicationConstain.AmqpNotifyMessageEventCode.ALARM.getCode());
        } catch (IOException e) {
